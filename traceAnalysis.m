function traceAnalysis(filename,hfile,plotFig) %Use filename
...
measurements = h5read(hfile,'/summary');
...
for i = 1:nFrames
    ...
    frame = i-1;
    first = b;
    while measurements.time(b) == frame && (b < length(measurements.time))
        b = b+1;
    end
    last = b-1;        %Plot the current frame
        whiskMov.cdata = read(vidobj,i);
    ...
    for j = 1:length(indList)
        ...
        whiskerCurve = measurements.curvature(t);
        whiskAngle =  faceAngle + abs(measurements.angle(t)) + 90;
